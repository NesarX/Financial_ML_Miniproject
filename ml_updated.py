...
# Rename USD cost columns to INR for consistency in the ML training (assuming the user will input INR values later)
rename_map = {}
...
model_input_cols = [
    "Initial_Cost_INR", # Use INR column created by renaming in step 1
...
# Get a safe list of available columns in the new data for the model
safe_input_cols = [col for col in model_input_cols if col in new_machine_data.columns]
model_input_df = new_machine_data[safe_input_cols].copy()
...
# --- 10. Financial Metrics Calculation (Using INR) ---
...
comparison_df['TCO_Per_Unit'] = (
    comparison_df['Effective_Initial_Cost']
    + (ANNUAL_OPERATING_COST_INR * TIME_HORIZON_YRS) # FIX: Use only INR
...
# ROI relative to effective initial cost. 
...
print(
    ...
    f"Annual Revenue/Unit=₹{TARGET_REVENUE_PER_UNIT_YR:,.0f}, "
    f"Annual Operating Cost/Unit=₹{ANNUAL_OPERATING_COST_INR:,.0f}"
)
